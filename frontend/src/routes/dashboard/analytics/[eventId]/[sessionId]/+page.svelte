<script lang="ts">
  ...
  function getStatusBadgeClass(rate: number): string {
    ...
    return 'bg-rose-500/15 text-rose-500 dark:text-rose-450';
  }
  ...
  async function generatePDFReport() {
    ...
    try {
      ...
      // High-resolution canvas-based Word Cloud image generator
      function generateHighResWordCloud(
        width: number,
        height: number,
      ...
      function drawHorizontalBarChart(
        ...
        chartData.forEach(item => {
          ...
          if (item.pct > 0) {
            ...
            if (item.isWinner) {
              pdfDoc.setFillColor(124, 58, 237); // Violet
            } else {
      ...
      if (mascotUri) {
        ...
        doc.setFillColor(124, 58, 237); doc.ellipse(logoX + 7.5, logoY + 7.5, 4, 4, 'F');
      ...
      doc.setFontSize(28); doc.setTextColor(124, 58, 237);
      ...
      doc.setFillColor(226, 232, 240); doc.rect(20, 93, pw - 40, 0.5, 'F');
      ...
      // Card 1: Slides
      doc.setFillColor(248, 250, 252); doc.roundedRect(20, 105, cardW, cardH, 2, 2, 'F');
      doc.setDrawColor(226, 232, 240); doc.setLineWidth(0.3); doc.roundedRect(20, 105, cardW, cardH, 2, 2, 'D');
      ...
      doc.setFillColor(248, 250, 252); doc.roundedRect(20 + cardW + cardGap, 105, cardW, cardH, 2, 2, 'F');
      ...
      // Card 3: Participants
      doc.setFillColor(248, 250, 252); doc.roundedRect(20, 105 + cardH + cardGap, cardW, cardH, 2, 2, 'F');
      ...
      // Card 4: Responses
      doc.setFillColor(248, 250, 252); doc.roundedRect(20 + cardW + cardGap, 105 + cardH + cardGap, cardW, cardH, 2, 2, 'F');
      ...
      slides.forEach((slide, idx) => {
        ...
        if (slide.type === 'POLL') {
          ...
          doc.setFontSize(9.5); doc.setFont('helvetica', 'bold'); doc.setTextColor(71, 85, 105);
      ...
      for (let i = 1; i <= totalPagesCount; i++) {
        ...
        if (i > 1) {
          ...
          doc.text('Generated by Rforum • rforum.t4gc.in', 15, ph - 10);
...
<main class="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
  ...
  {#if loading}
    <div class="flex flex-col items-center justify-center py-24 gap-3">
      ...
      <span class="text-sm text-surface-500 dark:text-surface-400 font-medium">Loading session analytics…</span>
    </div>
  ...
  {:else}
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 mb-8">
      {#each [
        ...
        { label: 'Slides',         value: slides.length,               icon: Layers,       bg: 'bg-brand-500/10 dark:bg-brand-500/15',     text: 'text-brand-500 dark:text-brand-400',    hover: 'hover:border-brand-500/30'   },
        ...
        { label: 'Engagement',     value: `${sessionEngagementRate}%`, icon: BarChart2,    bg: 'bg-accent-500/10 dark:bg-accent-500/15',   text: 'text-accent-500 dark:text-accent-400',  hover: 'hover:border-accent-500/30'  },
        { label: 'Avg / Person',   value: averageResponsesPerParticipant, icon: Activity, bg: 'bg-amber-500/10 dark:bg-amber-500/15',    text: 'text-amber-500 dark:text-amber-400',    hover: 'hover:border-amber-500/30'   },
      ] as card}
